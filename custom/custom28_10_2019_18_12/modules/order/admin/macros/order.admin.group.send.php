...
class Order_admin_group_send extends Diafan
{
    ...
    public function action()
    {
        ...
        if (count($ids) == 1) {
            ...
            /*
            $domPdfPath = realpath($_SERVER["DOCUMENT_ROOT"] . '/custom/custom28_10_2019_18_12/plugins/vendor/dompdf/dompdf');
            \PhpOffice\PhpWord\Settings::setPdfRendererPath($domPdfPath);
            PhpOffice\PhpWord\Settings::setPdfRendererName('DomPDF');
            $temp = PhpOffice\PhpWord\IOFactory::load($dir.'/doc_tmp.docx');

            $xmlWriter = PhpOffice\PhpWord\IOFactory::createWriter($temp, 'PDF');
            $xmlWriter->save($dir.'/doc.pdf');
            */
            ...
            $convert->fromURL('http://vend.dlay.ru/tmp/orders/doc_tmp.docx?v='.rand(1,9999));
            $convert->toFile($dir.'/demo.jpeg');

            $from_mail = EMAIL_CONFIG;
            $this->diafan->_postman->message_add_mail($from_mail, "Заголовок письма", 'Содержимое письма.<br>Можно использовать HTML. <img src="http://vend.dlay.ru/tmp/orders/demo2.jpeg?v='.rand(1,9999).'">');
